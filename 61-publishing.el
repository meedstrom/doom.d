...
;; record the id in the Post datatype.
;;
...
that org-id links will resolve correctly."
  ...
  ;; TODO: Just do this anyway for the source files henceforth so I can remove
  ;; this code snippet
  (cl-loop
  ...
  ;; WIP TODO: Ensure that the web links will contain a stable ID by putting
  ;; them in subfolders named by a shortened form of their org-id. REVIEW: does
  ;; it work?
  (let ((default-directory "/tmp/roam"))
    (cl-loop
     for file in (directory-files "." nil "\\.org$")
     as uuid = (->> (with-temp-buffer
                      (insert-file-contents-literally file)
                      (search-forward ":id: ")
                      (buffer-substring (point) (line-end-position)))
                    (string-replace "-" "")
                    (upcase))
     as short-id = (substring
                    (shell-command-to-string
                     (concat "echo " uuid " | basenc -d --base16 | basenc --base64url")
                     0 8))
     do (progn
          (mkdir short-id)
          (rename-file file short-id))))
